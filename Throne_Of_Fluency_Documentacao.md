...
| :---- | :---- |
| **Linguagem** | GDScript |
| **Resolução** | 800×760, stretch mode canvas\_items |
...
Os combates são em turnos, projetados para dar ao jogador tempo de pensar antes de agir. A exploração acontece no overworld, mas a batalha acontece em uma tela dedicada com fundo fixo, slots de personagens e UI inferior de comandos/status.
...
### **Fluxo atual de batalha**

O sistema atual já integra inimigos do overworld com a battle screen:

1. O inimigo existe no mapa e pode perseguir o player.
2. Ao entrar na `DangerBox`, o encontro é enviado para `BattleTransition`.
3. A cena muda para `battleSystem/battle_scene.tscn`.
4. A battle screen monta fundo fixo espacial, inimigo em slot superior e player em slot inferior.
5. O sistema usa os `SpriteFrames` reais do player e do inimigo, incluindo `idle`, `attack`, `dying` e `run_down`.
6. Vitória toca a animação `dying` dos inimigos mortos, marca o encontro como derrotado e retorna ao overworld.
7. Fuga toca `run_down` no player, retorna ao overworld e não remove o inimigo.
8. Derrota toca `dying` nos players mortos e segue para o fluxo provisório de game over/save anterior.

### **Comandos de batalha**

* **Attack** – ataque básico do personagem ativo.
* **Combo/Skills** – espaço reservado para evolução do sistema de habilidades e combos.
* **Item** – espaço reservado para inventário.
* **Run** – fuga funcional com animação de saída do player.
...
* Local: Cripta dos Ecos Perdidos (`world/cripta/cripta.tscn`)
...
Nesta fase, o herói desperta como uma armadura vazia ressuscitada. Lumen o encontra e explica sua missão. O herói encontra dois objetos: a Lâmina de Ferro Comum e o Grimório de Marfim. A porta de pedra da cripta será o primeiro teste de compreensão linguística.

## **Fase 2: Aventura em ShroomLands**

* Local: ShroomLands (`world/shroom-lands.tscn`)
...
* Validação técnica atual: inimigo de entrada perseguindo player e iniciando batalha fixa estilo JRPG.
...
* Game over real, save anterior, XP, loot e tela completa de vitória

*Documento atualizado para o fluxo atual de batalha e cenas reais do projeto.*

*Todas as informações do projeto compiladas e estruturadas | 2026*
